  ...
  "
    ...
    md:px-10 md:py-8">
          <div className="pb-3 md:pb-4">
            <NotificationBox message={notification.message} type={notification.type} showResend={notification.showResend} />
          </div>
          <p className="text-white font-bold text-2xl md:text-3xl mb-3">Register</p>

          <form onSubmit={submit} className="flex flex-col gap-4">
            {/* Email */}
            <div className="flex flex-col">
              <input type="text" placeholder="Name" value={name} onChange={(e) => { setName(e.target.value); setErrors((prev) => ({ ...prev, name: undefined })); }} className={`bg-black px-4 pt-5 pb-2 rounded-lg text-base md:text-lg border-b-2 transition-all duration-300 ${errors.email ? "border-red-500" : "border-white/60 focus:border-white"} focus:outline-none`} />
      ...
      focus:outline-none`}
            ...
            <div className="flex justify-between items-center text-xs md:text-sm text-white/80 mt-2">
              <span>Already have an account? <Link href="/login" className="text-[#D5E400] underline cursor-pointer">Log In</Link></span>
            </div>
      ...
      disabled:opacity-50" disabled={loading}>
          ...
          <Link href="/api/auth/google/login?role=user&intent=register" className=" relative flex mt-4 mx-auto
    bg-[#404811]/60
    px-10 md:px-24 py-3
    rounded-full
